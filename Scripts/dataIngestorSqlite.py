...
def insert_data_to_sqlite(conn, data, N):
    cursor = conn.cursor()
    ...
    for i in range(N):
        ...
        if i < len(data['branches']):
            ...
            cursor.execute("INSERT INTO branches (id, name, location, manager, state) VALUES (?, ?, ?, ?, ?)",
                           (branch["id"], branch["name"], branch["location"], branch["manager"], branch["state"]))

        if i < len(data['positions']):
            pos = data['positions'][i]
            cursor.execute("INSERT INTO positions (id, name, salary) VALUES (?, ?, ?)",
        ...
        if i < len(data['checkout']):
            co = data['checkout'][i]
            cursor.execute("INSERT INTO checkout (id, number, branch_id, employee_id) VALUES (?, ?, ?, ?)",
...
def main():
    ...
    if conn:
        ...
        insert_data_to_sqlite(conn, data, N)
        conn.close()
